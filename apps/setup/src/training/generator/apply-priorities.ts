...
// deprioritized areas) to every role-tagged slot, so later stages can rank
// exercise candidates and adjust prescribed volume — without changing
...
export function applyPriorities(
  ...
  return days.map((day) => ({
    name: day.name,
    slots: day.slots.map((slot) => ({ ...slot, preferredMuscles, avoidMuscles })),
  }));
}
